...
export class ChunkManager {
    ...
    for (let y = ccy - GameConfig.activeChunkRadius; y <= ccy + GameConfig.activeChunkRadius; y++) {
      for (let x = ccx - GameConfig.activeChunkRadius; x <= ccx + GameConfig.activeChunkRadius; x++) {
        ...
        if (!this.active.has(key)) {
          this.activateChunk(x, y, key, playerX, playerY, pellets, aiCells, pelletPool, aiPool);
        }
      }
    }
  ...
  private activateChunk(
    ...
    for (let i = 0; i < spawn.ai.length && aiCells.length < GameConfig.worldMaxAICount; i++) {
      const aiData = spawn.ai[i];
      const dx = aiData.x - playerX;
      const dy = aiData.y - playerY;
      if (dx * dx + dy * dy < GameConfig.spawnSafeRadius * GameConfig.spawnSafeRadius && aiData.mass > GameConfig.playerStartMass * 0.95) {
        continue;
      }
